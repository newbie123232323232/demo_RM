...
## Ba kênh test backend (demo Add Product) — Source: `Planning_doc_demo/Lessonlearn.md` §19

- **Kênh 1:** `dotnet test` + script `module_src/backend/api-tests/step-*-*.ps1` (agent/CI dán output).
- **Kênh 2:** Postman `module_src/backend/api-tests/postman/RevenueModule.postman_collection.json` (Collection Runner + Tests).
- **Kênh 3:** Manual (`.http`, Postman từng request, curl).
- FE không nằm trong bộ ba này trừ khi step yêu cầu rõ.

## Đồng bộ tiến độ checklist realtime (demo) — Source: `Planning_doc_demo/Lessonlearn.md` §20

- Có bằng chứng verify thì tick checklist ngay trong cùng phiên.
- Trạng thái “done nhưng chưa tick” được xem là drift quy trình; phải log issue rồi sửa checklist trước khi qua step tiếp theo.

---

*Cập nhật: `module_src`; khởi chạy local; format mục đích: lệnh; phương pháp test API ba kênh (script + Postman + manual).*
